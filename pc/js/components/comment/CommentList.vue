  ...
  <div id="normal-comment-list" class="normal-comment-list">
    ...
    <div id="comments">
      ...
      <div class="comments">
        <div
          v-if="user"
          v-clickoutside="onClickOutsideCommentEditor"
          :data-commentid="0"
          class="comment-source-box"
        >
        ...
        <div
          ...
          class="comment lastchild-flag"
          :key="`comment-${comment.id}`"
          :id="`comment-${comment.id}`"
        >
          <div
            class="comment-avatar-area v-tooltip-container"
          ...
          >
            <div class="v-tooltip-content">
              <a :href="`/uc/${comment.user.id}`" target="_blank" class="avatar">
                <img :src="comment.user.avatarURL" />
              </a>
              <UserBusinessCard
                v-if="comment.id === mouseenterCommentID && mouseenterCommentTarget === 'avatar'"
          ...
          <div class="comment-content-area">
            <div class="meta-box">
              ...
              >
                ...
                <UserBusinessCard
                  v-if="comment.id === mouseenterCommentID && mouseenterCommentTarget === 'username'"
                  :userID="comment.user.id"
                  :followerID="userID"
                />
              </div>
            ...
            <!-- 回复评论 -->
            <div
              v-if="user && comment.editorToggled"
              v-clickoutside="onClickOutsideCommentEditor"
              :data-commentid="comment.id"
              class="comment-root-box"
            >
              <div class="comment-source-box-wrap">
                <CommentRichEditor
                  :collectionID="collectionID"
            ...
            <div v-if="comment.comments && comment.comments.length" class="sub-comment-list">
              <div
                ...
                class="sub-comment"
                :style="{'padding-top': i === 0 ? '14px' : '12px'}"
              >
                <div
                ...
                >
                  <div
                    ...
                    @mouseleave="onMouseLeaveUser"
                  >
                    <a :href="`/uc/${subcomment.user.id}`" target="_blank" class="avatar">
                      <img :src="subcomment.user.avatarURL" />
                    </a>
                    <UserBusinessCard
                      v-if="subcomment.id === mouseenterCommentID && mouseenterCommentTarget === 'avatar'"
                ...
                <div class="subcomment-content-area">
                  <div class="meta-box">
                    <div
                      class="user-popover-box"
                      @mouseenter="onMouseEnterUser(subcomment.id, 'username')"
                      @mouseleave="onMouseLeaveUser"
                    ...
                    >
                      ...
                      <UserBusinessCard
                        v-if="subcomment.id === mouseenterCommentID && mouseenterCommentTarget === 'username'"
                        :userID="subcomment.user.id"
                        :followerID="userID"
                      />
                    </div>
                  ...
                  <div class="sub-content-box">
                    ...
                    <div
                      class="user-popover-box"
                      @mouseenter="onMouseEnterUser(subcomment.id, 'parent_username')"
                      @mouseleave="onMouseLeaveUser"
                      :style="{'z-index': subcomment.id === mouseenterCommentID && mouseenterCommentTarget === 'parent_username' ? visibleZIndex : defaultZIndex}"
                    >
                      <a
                        :href="`/uc/${subcomment.parent.user.id}`"
                        target="_blank"
                        class="username be-replied"
                      >
                        {{subcomment.parent.user.username}}
                        <a
                          :href="userLevelChapterURL"
                          target="_blank"
                          class="rank"
                      ...
                      <UserBusinessCard
                        v-if="subcomment.id === mouseenterCommentID && mouseenterCommentTarget === 'parent_username'"
                        :userID="subcomment.parent.user.id"
                        :followerID="userID"
                      />
                    </div>
                    <span>:</span>
                  ...
                  <div class="comment-operate" style="margin-bottom: 0;">
                    ...
                    <div class="comment-action-box">
                      <div
                        @click="onLikeOrNot(subcomment)"
                        class="like-action"
                        :class="{active: subcomment.userLiked}"
                      >
                  ...
                  <div
                    v-if="user && subcomment.editorToggled"
                    v-clickoutside="onClickOutsideCommentEditor"
                    :data-commentid="subcomment.id"
                    class="comment-sub-box"
                  >
                    <div class="comment-source-box-wrap">
                      <CommentRichEditor
                        :emptyPlaceholder="`回复${subcomment.user.username}`"
                        :collectionID="collectionID"
                        :sourceID="sourceID"
                        :rootID="comment.id"
                        ...
                        :source="source"
                        :disableInputBlur="true"
                        :ref="`commentEditor-${subcomment.id}`"
                      />
                    </div>
                  </div>
                </div>
              ...
              <!-- 一级评论的子评论未加载完时的加载按钮 -->
              <div
                v-if="comment.comments.length < comment.commentCount"
                @click="onLoadSub(comment)"
                class="sub-comment fetch-more"
              >
                <div
                  class="fetch-more-comment"
                >{{subCommentLoadStatusMap[comment.id] ? '正在加载...' : '查看更多 >'}}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- 一级评论未加载完时的加载按钮 -->
      <div
        v-if="comments.length < theRootCommentCount"
        @click="onLoadMore"
        class="fetch-more-comment"
      >{{isLoading ? '正在加载...' : '查看更多 >'}}</div>
...
export default {
  ...
  methods: {
    ...
    onLoadMore() {
      ...
      const lastCommentID = this.comments.length
      ...
      myHTTP
        ...
        .then(res => {
          ...
          if (
            this.isFirstRequest &&
            location.hash &&
            location.hash.indexOf('#comments') === 0
          ) {
            setTimeout(() => {
              // 滚动到评论区
              document
                .getElementsByClassName('comment-scroll-to')[0]
                .scrollIntoView();
            }, 50);
          }
    ...
    onLoadSub(parentComment) {
      ...
      const lastID =
        parentComment.comments[parentComment.comments.length - 1].id;
...
.comment-content-area .meta-box,
.subcomment-content-area .meta-box {
  display: flex;
  align-items: center;
  font-size: 14px;
  line-height: 1.2;
...
.comment-content-area .author .name:hover,
.subcomment-content-area .author .name:hover {
...
.comments .top-title .author-only:focus,
.comments .top-title .author-only:hover {
  text-decoration: none;
}
...
.comments .pull-right .active,
.comments .pull-right a:hover {
  color: #2f2f2f;
  text-decoration: none;
}
...
.normal-comment-list .close-comment,
.normal-comment-list .no-author-comment,
.normal-comment-list .no-comment {
  width: 226px;
  height: 92px;
  margin: 30px auto 20px;
}
...
.normal-comment-list .text a,
.normal-comment-list .text a:hover {
  color: #3194d0;
  text-decoration: none;
}
...
.normal-comment-list .comment-user-name,
.normal-comment-list .comment-user-name:hover {
