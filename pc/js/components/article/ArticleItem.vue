<template>
  <div class="article-list-item" :style="{'min-height': articleData.coverURL ? '145px' : '0'}">
    <Alert ref="delAlert" width="450" @ok="onDeleteAlertOk" @cancel="onDeleteAlertCancel" />
    <div class="article-content">
      <div class="article-content-and-img">
        <div class="article-title-summary">
          <a
            :href="`/p/${articleData.id}`"
            target="_blank"
            class="article-title"
            v-html="articleData.name"
          ></a>
          <p class="article-content" v-html="articleData.summary"></p>
        </div>
        <a
          v-if="articleData.coverURL"
          :href="`/p/${articleData.id}`"
          target="_blank"
          class="article-img"
        >
      ...
      <div class="article-meta">
        <div class="article-view-like">
          <a target="_blank" :href="`/p/${articleData.id}`">
            <i class="iconfont ic-list-read"></i>
            {{articleData.browseCount || 0}}
          </a>
          <a
            v-if="articleData.user && articleData.user.username"
            :href="`/uc/${articleData.user.id}`"
            target="_blank"
            class="article-username"
          >{{articleData.user.username}}</a>
          ...
          <a href="javascript:void(0)" style="color: #b4b4b4; cursor: default;">
            <img src="../../../images/article/zan.svg" style="width: 12px;" />
            {{articleData.likedCount || 0}}
          </a>
          ...
          >
            <div v-if="morePopupVisible" class="dropdown1">
              ...
              <ul class="dropdown-menu1">
                <li>
                  <a :href="`/editor/posts/${articleData.id}`">编辑</a>
                </li>
...
export default {
  props: ['keyword', 'article', 'moreBtnVisible'],
  data() {
    ...
    if (this.keyword) {
      articleData.name = replaceIgnoreCase(
        articleData.name,
        this.keyword,
        strongHTML,
      );
      articleData.summary =
        replaceIgnoreCase(articleData.summary, this.keyword, strongHTML) +
        '...';
    }
    return {
      articleData,
  ...
  methods: {
    ...
    onDelete() {
      this.$refs.delAlert.show(
        '提示',
        '删除文章会扣除相应的阅读值，且文章不可恢复。',
      );
    },
    onDeleteAlertOk() {
      myHTTP
        .delete(`/articles/${this.articleData.id}`)
        .then(res => {
