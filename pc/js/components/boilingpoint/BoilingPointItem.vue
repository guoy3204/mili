  ...
  <li class="boilingpoint-item">
    <div class="boilingpoint-item-pin">
      <div class="pin-header-row">
        <div class="account-group">
          <div
            ...
            @mouseenter="onMouseEnterUser1"
            @mouseleave="onMouseLeaveUser1"
          >
            <a :href="`/uc/${boilingData.user.id}`" target="_blank" class="user-link">
              <div
                class="lazy avatar loaded"
                :style="{'background-image': `url(${boilingData.user.avatarURL})`}"
              ></div>
            </a>
            <UserBusinessCard
              v-if="userCardVisible1"
              :userID="boilingData.user.id"
              :followerID="userID"
          ...
          <div class="pin-header-content">
            <div
              class="user-popover-box"
              @mouseenter="onMouseEnterUser2"
              @mouseleave="onMouseLeaveUser2"
            >
              <a
                :href="`/uc/${boilingData.user.id}`"
                target="_blank"
                class="username"
              >{{boilingData.user.username}}</a>
              <UserBusinessCard
                v-if="userCardVisible2"
                :userID="boilingData.user.id"
                :followerID="userID"
                @userFollowChange="onFollowChange"
              />
        ...
        <div class="header-action">
          <FollowBtn
            v-if="userID !== boilingData.user.id"
            @userFollowChange="onFollowChange"
            :userID="boilingData.user.id"
            :followed="isFollowed"
            :followedStyle="followedStyle"
            :notFollowedStyle="notFollowedStyle"
          />
      ...
      <div class="pin-content-row">
        <div
          v-if="isContentExpand || !partialContent"
          class="boilingpoint-content-box"
          v-html="boilingData.htmlContent"
        ></div>
        <div v-else class="boilingpoint-content-box" v-html="partialContent"></div>
        <div v-if="partialContent" class="content-limit-box">
          <div
            @click="changeContentExpand"
            class="content-limit-btn"
          >{{!isContentExpand ? '展开' : '收起'}}</div>
        </div>
      </div>
      <div class="pin-image-row">
        <div class="image-box-wrapper image-box">
          <div v-show="gridVisible" class="image-box" :style="{width: gridTotalWidth + 'px'}">
            <div
              @click="onClickImgGrid(i)"
              :key="imgData.id"
              v-for="(imgData, i) in imgArr"
              class="image"
              :class="{'no-right-margin': (i + 1) % 3 === 0}"
          ...
          <div
            v-if="middleImgCreated"
            :style="{width: '100%', display: middleImgVisible ? 'block' : 'none'}"
          >
            <div class="action-bar">
              <div
                @click="onExitMiddleImage"
                class="action-item"
                @mouseenter="() => pinchIconOver = true"
                @mouseleave="() => pinchIconOver = false"
              >
                <span class="icon">
              ...
              <div
                @click="onBrowseBigImage"
                class="action-item"
                @mouseenter="() => expendIconOver = true"
                @mouseleave="() => expendIconOver = false"
              >
                <span class="icon">
              ...
              <div
                @click="changeMiddleImgTransform(-1)"
                class="action-item"
                @mouseenter="() => leftRotateIconOver = true"
                @mouseleave="() => leftRotateIconOver = false"
              >
                <span class="icon">
                  <img
                    v-if="leftRotateIconOver"
                    src="../../../images/boilingpoint/leftrotateover.svg"
                  />
              ...
              <div
                @click="changeMiddleImgTransform(1)"
                class="action-item"
                @mouseenter="() => rightRotateIconOver = true"
                @mouseleave="() => rightRotateIconOver = false"
              >
                <span class="icon">
                  <img
                    v-if="rightRotateIconOver"
                    src="../../../images/boilingpoint/rightrotateover.svg"
                  />
            ...
            <div
              v-if="curMiddleImg"
              class="carousel-body"
              :style="{height: curMiddleImg.style.displayHeight}"
            >
              <img
                :key="middleImg.id"
                v-for="(middleImg, i) in middleImgArr"
                :src="middleImg.url"
                class="carousel-image animated myfadeIn fast"
                :class="{'hide-middle-img': i !== curImgIndex}"
              ...
              <div
                v-show="curImgIndex < middleImgArr.length - 1"
                @click="nextMiddleImg"
                class="toggle-area next"
              ></div>
            </div>
            <div class="nav-list">
              <div
                @click="changeMiddleImg(i)"
                :key="imgData.id"
                v-for="(imgData, i) in imgArr"
                class="nav-item"
              >
      ...
      <div v-if="boilingData.topic" class="pin-topic-row">
        <a
          :href="`/boilings/topic/${boilingData.topicID}`"
          target="_blank"
          :title="boilingData.topic.name"
          class="topic-title"
        >{{boilingData.topic.name}}</a>
      </div>
        ...
        <div class="action-box action-box">
          ...
          <div class="like-action action">
            <div @click="onLikeOrNot(boilingData)" class="action-title-box">
              ...
              <span
                class="action-title"
                :style="{color: boilingData.userLiked ? '#37c700' : '#8a93a0'}"
              >{{likedCount ? likedCount : '赞'}}</span>
            </div>
          </div>
          <!-- 圈子的评论 -->
          ...
          <div class="share-action action">
            <div
              @click="changeShareVisible"
              v-clickoutside="clickoutsideShare"
            ...
            >
              ...
              <Share
                v-if="shareVisible"
                :boilingPointItemID="boilingData.id"
                :summary="boilingData.summary"
                :imgs="imgArr"
                @copyLink="onCopyLink"
              />
            </div>
      ...
      <div v-if="iSCommentsVisible" class="boilingpoint-comments-box article">
        <div class="comment-list">
          <CommentList
            ...
            source="boilingpoint"
            :sourceID="boilingData.id"
            :user="user"
            :authorID="boilingData.user.id"
            :rootCommentCount="boilingData.rootCommentCount"
          />
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    ...
    middleImgArr = middleImgArr.map(img =>
      this.updateMiddleImgStyle(img, {
        isSwap: false,
    ...
    return {
      likedCount: this.boilingData.likedCount || 0, // 圈子获得的点赞数
      ...
      isFollowed: this.boilingData.user.isFollowed, // 是否已关注圈子作者
  ...
  methods: {
    ...
    updateMiddleImgStyle(imgData, options) {
      ...
      options.transform =
        options.transform || 'rotate(0deg) translate(0px, 0px)';
      const maxMiddleImageWidth =
        options.maxMiddleImageWidth || this.maxMiddleImageWidth;
      ...
      if (originalWidth > maxMiddleImageWidth) {
        ...
        imgSize[imgHeightKey] =
          originalHeight / (originalWidth / imgSize[imgWidthKey]);
      } else {
      ...
      if (imgSize[imgHeightKey] > 1000) {
        ...
        imgSize[imgWidthKey] =
          originalWidth / (originalHeight / imgSize[imgHeightKey]);
      }
      imgData.style = {
...
.nav-item .thumb.active,
.nav-item .thumb:hover {
  border-color: #ea6f5a;
  opacity: 1;
}
