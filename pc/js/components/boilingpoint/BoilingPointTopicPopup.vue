<template>
  <div @click="onClick" class="topic_panel">
    ...
    <div class="top">
      ...
      <div
        class="lazy search_icon loaded immediate"
        :style="{'background-image': 'url(../../../images/boilingpoint/search.svg)'}"
      ></div>
    </div>
    <div class="content">
        ...
        <li @click="onClickTopic(null)">
          <div class="box">
            <div
              ...
              :style="{'background-image': 'url(../../../images/boilingpoint/notopic.svg)'}"
            ></div>
          </div>
          <span>不添加任何话题</span>
        </li>
      ...
      <ul v-if="curTopics.length" class="topic-list">
        <li @click="onClickTopic(topic)" :key="topic.id" v-for="topic in curTopics">
          <div class="topic-item">
            <div
              class="lazy icon loaded immediate"
              :style="{'background-image': `url(${topic.icon})`}"
            ></div>
            <div class="topic-content">
              <span>{{topic.name}}</span>
              <span>246 关注 · 17 圈子</span>
            </div>
          </div>
        </li>
      </ul>
      <div v-if="isLoading" class="loading">
        <div
          class="lazy empty-icon loaded immediate"
          style="background-image: url(../../../images/boilingpoint/loading.gif);"
        ></div>
      </div>
      <div v-if="isEmpty" class="empty">
        <div
          class="lazy empty-icon loaded immediate"
          style="background-image: url(../../../images/boilingpoint/empty.svg);"
        ></div>
        <span>搜索列表为空</span>
      </div>
    </div>
...
.topic_panel .content .empty,
.topic_panel .content .loading {
  width: 100%;
  height: 364px;
  display: flex;
  flex-direction: column;
...
.topic_panel .content .empty span,
.topic_panel .content .loading span {
  padding: 7px 0;
  font-size: 15px;
  color: #b2bac2;
  opacity: 0.8;
