<template>
  <div>
    <!-- <Upload :action="uploadActionURL"
            :on-success="onUploadCallback" :header="uploadHeaders" :on-error="onUploadError" :name="uploadFieldName"
...
import { aliyunOssResponseParse } from '~/js/utils/utils.js';
...
export default {
  ...
  data() {
    ...
    return {
      ...
      uploadHeaders: {
        'x-csrf-token': uploadPolicy.csrfToken,
      },
    };
  },
  mounted() {
      ...
      axios.post(this.uploadActionURL, form, config).then(res => {
        ...
        if (uploadPolicy.ossType === 'fs') {
          ...
          if (process.env.NODE_ENV === 'development') {
            ...
            let imgData = aliyunOssResponseParse(res, this.uploadImgURL);
            if (imgData.url) {
              const url = `/common/oss/createimg`;
              myHTTP
                .post(url, { key: imgData.path })
                .then(res => {
