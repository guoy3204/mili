      ...
      <div v-if="searchTipVisible" id="navbar-search-tips">
        <div class="search-trending">
          <div class="search-trending-header clearfix">
            <span>热门搜索</span>
            <a>
              <i class="iconfont ic-search-change" style="transform: rotate(0deg);"></i> 换一批
            </a>
          </div>
          <ul class="search-trending-tag-wrap">
            <li :key="trending.id" v-for="trending in trendingArr">
              <a
                :href="`/search?q=${encodeURIComponent(trending.label)}`"
                target="_blank"
              >22{{trending.label}}</a>
            </li>
          </ul>
...
export default {
  ...
  data: function() {
    return {
      ...
      searchKeyword: '',
      trendingArr: [{ label: '毕业' }, { label: '故事' }, { label: '理财' }],
      recentArr: [{ label: 'vue' }, { label: 'java' }],
    };
  },
  computed: {
    encodedKeyword: {
