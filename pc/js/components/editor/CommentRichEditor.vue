<template>
  <div class="comment-editor-box">
    ...
    <div
      @keyup.meta.enter="onEnterSubmit"
      @keyup.ctrl.enter="onEnterSubmit"
      class="comment-editor-cbox"
      :class="{'comment-editor-cbox-boilingpoint': source === 'createboilingpoint', 'comment-editor-focus': isFocus}"
    >
      <editor-content class="cms-editor-content" :editor="editor" />
      <div
        v-if="source === 'createboilingpoint'"
        @click="onFocusAreaClick"
        class="editor-focus-area"
      ></div>
    </div>
    <slot name="upload-list"></slot>
    <div
      v-if="sendVisible"
      class="write-function-block"
      :style="{height: source === 'createboilingpoint' ? '50px' : '0'}"
    >
      ...
      <template v-if="source === 'createboilingpoint'">
        <button
          @click="onEnterSubmit"
          class="btn btn-send-boilingpoint"
          :class="{active: boilingpointSubmitEnable}"
        >发布</button>
...
export default {
  ...
  data() {
    return {
      sendVisible:
        typeof this.sendDefVisible === 'undefined' ? true : this.sendDefVisible,
      editor: new Editor({
        extensions: [
          new Image(),
          new Placeholder({
  ...
  methods: {
    ...
    onBoilingpointSubmit() {
      ...
      // 抛出事件，由 BoilingPointEditor.vue 中处理发布圈子
      this.$emit('submit');
    },
    onTopicClick() {
...
.cms-editor-content p.is-empty {
  margin: 0;
}

.cms-editor-content p.is-empty:first-child::before {
