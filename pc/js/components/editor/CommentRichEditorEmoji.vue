    ...
    <div v-clickoutside="onClickOutside" class="helloman" slot-scope="{ commands, }">
      <template>
        ...
        <a
          ...
          class="up-img"
          :class="{'not-allowed': !uploadAllowed}"
        >
          <Uploader
            v-if="uploadAllowed"
        ...
        <a
          v-if="source === 'createboilingpoint'"
          @click="onTopicClick"
          v-clickoutside="onCloseTopicPopup"
          class="topic"
        ...
        >
          ...
          <div class="label" style="position: relative;">
            ...
            <BoilingPointTopicPopup
              ...
              @close="onCloseTopicPopup"
              v-if="topicPopupVisible"
            />
          </div>
        </a>
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      tabs: [0, 1, 2, 3],
      emojiVisible: false,
      topicPopupVisible: false, // 是否显示圈子的话题搜索框
    };
  },
  methods: {
    changeTab(index) {
...
.comment-editor-box .emoji-modal-wrap .modal-header li.active a,
.comment-editor-box .emoji-modal-wrap .modal-header a:hover {
  background: #2f2f2f;
}
...
.comment-editor-box .not-allowed,
.comment-editor-box .not-allowed .label {
