        ...
        <div class="inner">
            ...
            <div class="star-content">
              ...
              <div class="star-panel">
                <div
                  ...
                  v-for="i in 5"
                  class="star"
                  :class="{'star-selected': starCount && i <= starCount}"
                ></div>
          ...
          <div class="bottom">
            <button
              @click="onCommit"
              class="submit-btn"
              :class="{'submit-disabled': !submitEnabled}"
            >提交评价</button>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    show() {
      if (!this.user) {
        ...
        let ref =
          location.href +
          (hasQuery ? '&star_modal_visible=true' : '?star_modal_visible=true');
        location.href = '/signin?ref=' + encodeURIComponent(ref);
        return;
      }
...
.handbook-modal .inner .top .aside img {
  width: 100%;
  display: block;
  box-shadow: 0 10px 25px 0 rgba(93, 93, 93, 0.3);
}
...
.handbook-modal textarea {
  ...
  font-size: 16px;
  outline: none;
  border: 1px solid rgba(93, 112, 132, 0.2);
  color: #5d7084;
  resize: none;
}
