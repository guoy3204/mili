          ...
          <div class="top">
            <h1 class="username">
              ...
              <a href="/" target="_blank" class="rank">
                <img :src="author.level | levelImgURL" />
              </a>
            </h1>
          </div>
          <div class="position">
          ...
          <div class="list-header">
            <div class="header-content">
              ...
              <router-link :to="`/uc/${author.id}/boilings`" class="nav-item" active-class="active">
                <div class="item-title">圈子</div>
                <div class="item-count">{{author.boilingPointCount}}</div>
              </router-link>
              <div
                @click="onLikeClick"
                v-clickoutside="onClickOutSide"
                class="nav-item not-in-scroll-mode"
              ...
              >
                ...
                <div v-if="likeClicked" class="more-panel">
                  <router-link
                    :to="`/uc/${author.id}/like/articles`"
                    class="more-item"
                  >文章 {{author.articleLikeCount || 0}}</router-link>
                  <router-link
                    :to="`/uc/${author.id}/like/boilings`"
                    class="more-item"
                  >圈子 {{author.boilingPointLikeCount || 0}}</router-link>
                </div>
              </div>
              <router-link
                :to="`/uc/${author.id}/follows`"
                class="nav-item"
                :class="{active: isFollowPage}"
              >
                <div class="item-title">关注</div>
              </router-link>
              <router-link
                :to="`/uc/${author.id}/buyhandbooks`"
                class="nav-item"
            ...
            <div class="stat-item">
              <svg width="25" height="26" viewBox="0 0 25 26" class="zan-icon">
                <g fill="none" fill-rule="evenodd" transform="translate(0 .57)">
                  <ellipse cx="12.5" cy="12.57" fill="#fadcd6" rx="12.5" ry="12.57" />
                  <path
                    fill="#e77c6a"
              ...
              <span class="content">
                获得点赞
                <span class="count">{{author.likedCount | prettyCount}}</span>
              </span>
            </div>
            <div class="stat-item">
              <svg width="25" height="25" viewBox="0 0 25 25" class="article-view-icon">
                <g fill="none" fill-rule="evenodd">
                  <circle cx="12.5" cy="12.5" r="12.5" fill="#fadcd6" />
                  <path
                    fill="#e77c6a"
              ...
              <span class="content">
                文章被阅读
                <span class="count">{{author.articleViewCount | prettyCount}}</span>
              </span>
            </div>
            <div class="stat-item">
              ...
              <span class="content">
                阅读值
                <span class="count">{{author.value | prettyCount}}</span>
              </span>
            </div>
          </div>
        </div>
...
export default {
  data() {
    return {
      likeClicked: false, // tab中的赞是否已点击，点击了的话，弹出文章、圈子下拉列表
...
.list-header .nav-item:not(.active):not(.open):hover .item-count,
.list-header .nav-item:not(.active):not(.open):hover .item-title {
  opacity: 0.8;
}
...
.list-header .nav-item.active .item-count,
.list-header .nav-item.active .item-title {
  color: #ea6f5a;
}
