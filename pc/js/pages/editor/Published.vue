  ...
  <div>
    ...
    <div class="done-header">
      <div class="done-title">
        <a :href="`/p/${article.id}`" class="main-title">{{article.name}}</a>
        <br />
      ...
      <ul class="article-share">
        ...
        <li @click="showQRCodePopup" class="weixin">
          <i class="fa fa-wechat"></i>微信
        </li>
        <li class="link" :data-clipboard-text="`${url}/p/${article.id}`" @click="onCopyLink">
      ...
      <div class="done-search">
        <div class="done-search-input">
          ...
          <input
            ...
            @input="onSearchInput"
            @keyup.enter="onSearchCollection"
            type="text"
            placeholder="搜索专题"
          />
        </div>
          ...
          <div v-if="isSearchLoading" class="loading-collection-box">
            <div class="loading-rect">
              <svg class="loading-collection" viewBox="25 25 50 50">
                <circle class="loading-collection-circle" cx="50" cy="50" r="20" fill="none" />
              </svg>
            </div>
          </div>
          <div v-else>
            <div v-if="searchCollections && searchCollections.length" class="collection-box">
              <ul class="collection-list" style="border-top: 1px solid #f2f2f2;">
                <li
                  :key="c.id"
                  v-for="c in searchCollections"
                  class="collection-item"
                  style="width: 50%;"
                >
                  <img @click="jumpToCollection(c.id)" :src="c.coverURL" />
                  <a
                    :style="{cursor: c.buttonMode ? 'pointer' : 'default'}"
                    @click="onContribute(c)"
                  >{{c.statusLabel}}</a>
                  <span :title="c.name" class="recommend-name">
                    <span @click="jumpToCollection(c.id)">{{c.name}}</span>
                    <em>{{c.articleCount | countToK}}篇文章，{{c.followerCount | countToK}}人关注</em>
                  </span>
                </li>
              </ul>
            </div>
      ...
      <template v-else>
        <div class="collection-box my-collection-box">
          <h3 class="collection-box-title my-collection-box-title">
            创建/管理的专题
            <a class="my-collection-box-new" href="/collections/new">新建</a>
          </h3>
          <ul class="collection-list">
            <li :key="c.id" v-for="c in collections" class="collection-item">
              ...
              <a
                :style="{cursor: c.buttonMode ? 'pointer' : 'default'}"
                @click="onContribute(c, true)"
              >{{c.statusLabel}}</a>
            </li>
          </ul>
        </div>
        <div class="collection-box my-collection-box">
          <h3 class="collection-box-title my-collection-box-title">最近投稿</h3>
          <ul class="collection-list">
        ...
        <div class="collection-box" style="margin-bottom: 30px;">
          ...
          <ul class="collection-list">
            <li
              :key="c.id"
              v-for="c in recommendCollections"
              class="collection-item"
              style="width: 50%;"
            >
              <img @click="jumpToCollection(c.id)" :src="c.coverURL" />
        ...
        .then(res => {
          if (res.data.errorCode === ErrorCode.SUCCESS.CODE) {
            if (res.data.data.status === ArticleCollectionStatus.Collected) {
              ...
              res.data.data.status === ArticleCollectionStatus.Auditing
            ) {
              if (!isManager) {
...
.loading-collection-circle {
  ...
  stroke: #ec7259;
  -webkit-animation: strokedash 1.5s cubic-bezier(0.645, 0.045, 0.355, 1)
    infinite;
  animation: strokedash 1.5s cubic-bezier(0.645, 0.045, 0.355, 1) infinite;
