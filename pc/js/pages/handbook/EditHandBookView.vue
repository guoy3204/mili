<template>
  <div id="app">
    <Alert ref="delChapterAlert" width="450" @ok="onDelChapterOk" @cancel="onDelChapterCancel" />
    <AddChapterAlert
      ref="chapterTitleAlert"
      width="450"
      @ok="onChapterTitleOk"
      @cancel="onChapterTitleCancel"
    />
    <div id="editorBox">
      <HandbookHeader
        ref="handbookHeader"
      ...
      <div class="edit-wrap">
        <div class="directory-box">
          <div
            @click="onIntroduceClick"
            class="summary"
            :class="{'route-active': isIntroduceSelected}"
          >小书介绍</div>
...
export default {
  data() {
    return {
      ...
      handbook: null,
      isIntroduceSelected: true, // 是否选中小书介绍
      chapters: [],
      chapterMap: {},
  ...
  mounted() {
    ...
    Promise.all([
      ...
      chapterID !== 'introduce'
        ? this.requestChapterDetail(chapterID)
  ...
  methods: {
    ...
    onIntroduceClick() {
      ...
      // this.curChapter 为空，是小书介绍 切到 小书介绍, 直接返回
      if (!this.curChapter) {
        return;
      }
      // this.curChapter 不为空，是由 章节 切到 小书介绍
      ...
      if (this.isIntroduceSelected) {
        if (content === this.handbook.introduce) {
          return Promise.resolve({
            data: { errorCode: ErrorCode.SUCCESS.CODE },
          });
        }
    ...
    onCreateChapter(chapter, index) {
      this.$refs.chapterTitleAlert.show(
    ...
    onChapterTitleOk(data) {
      ...
      if (data.chapterID) {
        ...
        myHTTP
          .put(url, { name: data.chapterTitle })
          .then(res => {
            if (res.data.errorCode === ErrorCode.SUCCESS.CODE) {
      ...
      // 创建章节时，先保存当前选中的小书介绍或章节，再选中新创建的章节
      this.isLoading = true;
      this.saveIntroduceOrChapter()
        .then(res => {
          if (res.data.errorCode !== ErrorCode.SUCCESS.CODE) {
          ...
          this.isLoading = true;
          myHTTP
            .post(url, { name: data.chapterTitle })
            .then(res => {
              if (res.data.errorCode === ErrorCode.SUCCESS.CODE) {
                const newChapter = {
                  id: res.data.data.id,
    ...
    onDeleteChapter(id) {
      ...
      this.$refs.delChapterAlert.show(
        '删除章节',
        '此操作将删除该章节的内容，不可恢复，确定删除吗?',
      );
    },
    onDelChapterOk() {
    ...
    onUpdateChapterTryRead(chapter, i) {
      ...
      this.isLoading = true;
      myHTTP
        .put(url, { tryRead: !chapter.tryRead })
        .then(res => {
          if (res.data.errorCode === ErrorCode.SUCCESS.CODE) {
