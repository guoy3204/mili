  ...
  <div class="edit-book-area" :style="{height: editAreaHeight + 'px'}">
    <div class="book-top-edit">
      ...
      <div class="book-chapter-create">
        ...
        <Button
          v-if="book"
          :type="book.status === BookStatus.BookUnpublish ? 'primary' : 'error'"
    ...
    <div class="chapter-area">
      ...
      <div
        class="book-chapter-editor"
        :style="{'padding-top': contentType === ArticleContentType.HTML ? 0 : '10px'}"
      >
        ...
        <template v-else>
          <div v-if="contentType === ArticleContentType.HTML" class="rich-editor-body-box-wrap">
            <RichEditor title content :uploadPolicy="uploadPolicy" ref="richEditor" mode="admin" />
          </div>
          <div v-else class="md-editor-body-box-wrap">
            <MarkdownEditor :noPaddingTop="true" ref="mdEditor" />
          </div>
          ...
          <div class="curchapter-name-box">
            ...
            <div style="flex: 1;"></div>
            <div
              v-if="contentType === ArticleContentType.HTML"
              @click="onShowCrawModal"
              class="book-content-crawl-btn"
            >抓取内容</div>
      ...
      <CreateOrUpdateChapterModal
        ref="createOrUpdateChapterModal"
        :bookID="bookID"
        @chapter-created="onChapterCreated"
        @chapter-updated="onChapterUpdated"
      />
...
export default {
  ...
  computed: {
    publishBtnLabel() {
      ...
      return this.book.status === this.BookStatus.BookPublished
        ? '下架图书'
        : '发布图书';
    },
  },
  mounted() {
    console.info('Editbook mounted');
    this.isMounted = true;
    Promise.all([
    ...
    this.headerBoxHeight = document.getElementsByClassName(
      'header-box',
    )[0].offsetHeight;
  ...
  methods: {
    reqPolicy() {
      ...
      return myHTTP
        ...
        .catch(err => {
          console.warn('EditBook error', err);
        });
    },
    onWindowResize() {
    ...
    setRichEditorContentHeight() {
      ...
      document.getElementsByClassName('rich-editor-body-box-wrap')[0]
        .offsetHeight;
      this.$refs.richEditor.setContentHeight(
        this.editAreaHeight - 60 - 46 - 16 - 16 - 42,
      );
    },
    getBookChapterContent() {
      ...
      return myHTTP
        .get(
          `/books/chapters/${this.curChapter.id}/editorcontent?contentType=${this.contentType}`,
        )
        .then(res => {
            ...
            [
              ...
              h('Button', {
                ...
                style: {
                  marginRight: '8px',
                  display:
                    data.depth >= this.maxDepth ? 'none' : 'inline-block',
                },
    ...
    remove(root, node, data) {
      ...
      this.$Modal.confirm({
        ...
        onOk() {
          myHTTP
            .delete(`/admin/books/${self.bookID}/chapters/${data.id}`)
            .then(res => {
              if (res.data.errorCode !== ErrorCode.SUCCESS.CODE) {
    ...
    onPublishBookClick() {
      ...
      const self = this;
      const tip =
        self.book.status === self.BookStatus.BookUnpublish ? '发布' : '下架';
      this.$Modal.confirm({
        title: tip,
        content: `确定要${tip}?`,
        closable: true,
    ...
    onSaveChapterContent() {
      ...
      myHTTP
        .put(`/admin/books/chapters/${this.curChapter.id}`, data)
        .then(res => {
          if (res.data.errorCode === ErrorCode.SUCCESS.CODE) {
