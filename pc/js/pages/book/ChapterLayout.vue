        ...
        <div class="book-directory bought">
          ...
          >
            <div
              v-if="stepVisible"
              class="step"
              :class="{'section-read': i <= chapterIndex, 'section-current': i === chapterIndex}"
            >
    ...
    <div class="book-content">
      <div class="book-content-inner">
        ...
        <div class="book-handle book-direction">
          ...
          <div
            v-if="starModalEnabled && isFinalChapter"
            @click="onStarModal"
            class="step-btn step-btn--next step-btn--finished"
          >
...
export default {
  data() {
    ...
    return {
      ...
      book: window.book,
      prevChapter: getPrevNode(
        getTreeNode(chapterID, root.children),
        root.children,
      ),
      nextChapter: getNextNode(
        getTreeNode(chapterID, root.children),
        root.children,
      ),
      user: window.user,
  ...
  mounted() {
    ...
    this.$router.afterEach((to, from) => {
      ...
      if (chapterID !== this.chapterID) {
        ...
        this.prevChapter = getPrevNode(
          getTreeNode(chapterID, this.chapterTree),
          this.chapterTree,
        );
        this.nextChapter = getNextNode(
          getTreeNode(chapterID, this.chapterTree),
          this.chapterTree,
        );
        let chapterIndex = 0;
